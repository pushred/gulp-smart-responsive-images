  ...
  const stream = through.obj(function (file, enc, callback) {
    sharp(file.contents)
      ...
      .then(metadata => {
        ...
        const queue = widths.map(width => resizeImage(file, width, options.cropRatio));
